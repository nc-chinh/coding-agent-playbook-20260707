...
```
   ...
   本番 (target): api の adapter を provider 別実装に DI 差し替え、mock service は消える
...
| port | 責務 | 主なメソッド（イメージ） |
|---|---|---|
| `PowerDataPort` | 協会 power-data 相当。30 分値・契約マスタ取得 | `fetchReadings(consent: ConsentRef, range)` / `fetchContract(consent: ConsentRef)` |
...
`SmsPort` は `requestId` を返し、**試行回数・有効期限・送信頻度/レート制限**（電話番号・IP 単位）を契約に含める。MVP は mock が `requestId` にバインドした固定 OTP でのみ検証成功（デモ用。`requestId`/code が合わなければ失敗）だが、この制御責務は本番で SMS provider または api 側の小さな counter store（後述の target 状態）に委譲する前提を明記する（[§10](#10-セキュリティ--プライバシー)。ステートレス方針は診断本体に閉じ、認証のレート制御は例外）。
...
- **port が吸収すべき provider 差分**: 各 provider は認証（会員鍵 / OAuth / API key）、**同意/申込 ID の受け渡し**、同期/非同期（協会は取得が非同期になり得る）、エラー分類、レート制限、冪等キー・リトライが異なる。port の signature はこれらを引数/戻り値に含められる形にし（例: `fetchReadings(consent: ConsentRef, range)`、`ConsentRef` は内部の `consentId`。本番 adapter が同意記録から協会申込 ID（`providerRequestId`）を解決して実 API を叩く）、adapter 差し替えで吸収する。「URL 差し替えのみ」ではないことを前提に設計する
...
- `POST /sms/verify` `{ requestId, code }` → 固定 OTP（例 `123456`）のみ成功、他は 4xx（`SmsPort.verifyOtp(requestId, code)` 契約に合わせ `requestId` バインド。レート・失敗の分岐をデモできる）
...
web            api                         mock (協会/SMS/JEPX)
 ...
 │◀─ 200 { token: JWT(verified) } ────────│   ← 電話の pseudonymous ID・検証済 を claim 化
 │  同意画面（提供先/目的/期間を表示）       │
...
| api↔mock | mock は独立 HTTP service（実 API 形状を模倣）。api の adapter が HTTP で叩く。本番は同一 port への provider 別 adapter を DI 差し替え（URL 差し替えのみではない） |
