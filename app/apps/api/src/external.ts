...
function failExternal(path: string, startedAt: number, kind: ExternalFailureKind): ExternalError {
  const durationMs = Date.now() - startedAt;
  const safePath = logPath(path);
  console.error(JSON.stringify({ level: 'error', event: 'external_call', path: safePath, durationMs, kind }));
...
async function fetchExternal(path: string, init?: RequestInit): Promise<Response> {
  ...
  if (!res.ok) throw failExternal(path, startedAt, 'upstream');
  return res;
}
...
export const external = {
  ...
  async readings(consentId: string): Promise<ConsumptionReading[]> {
    ...
    return parseUpstream(zReadingsResponse, await getJson(path), path).readings;
  },
